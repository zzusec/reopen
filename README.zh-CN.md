<div align="center">
  ...
  <p>
    <a href="https://github.com/zzusec/reopen/releases/latest"><img src="https://img.shields.io/github/v/release/zzusec/reopen?label=release" alt="最新版本"></a>
    <a href="https://github.com/zzusec/reopen/actions/workflows/ci.yml"><img src="https://github.com/zzusec/reopen/actions/workflows/ci.yml/badge.svg" alt="CI 状态"></a>
...
> **Fork 说明与鸣谢**
>
> `reopen` 是 fork 自 [**haowang02**](https://github.com/haowang02) 的 [**agent-session-cleaner**](https://github.com/haowang02/agent-session-cleaner)。会话浏览、搜索、清理、归档等全部基础能力都是原作者的工作——没有它就没有这个项目。🙏
>
> `reopen` 唯一新增的是**一键恢复**：在浏览器里选中会话，按 `Enter`，当前进程就被 `claude --resume <id>` / `codex resume <id>` 替换，加载该会话的完整历史，让你从上次离开的地方接着干。这个功能来源于：自带 `/resume` 选择器在第三方 provider 下渲染异常、读不出会话列表。
>
> 采用 MIT 协议，保留原作者版权声明。
...
安装脚本会装上 `reopen`，并创建指向它的短别名 `re`。Windows 上会装到 `%LOCALAPPDATA%\Programs\reopen\bin` 并加入用户 `PATH`。首次安装后请打开一个新终端。
...
在 PowerShell 中，可先执行 `$env:REOPEN_LANG = "zh-CN"`，再运行 `reopen`。
...
这就是 `reopen` 的用途。把光标移到任意 Claude Code 或 Codex 会话上，按 `Enter`：当前进程被对应 Agent 的恢复命令替换，加载该会话的完整历史，让你接着上次的任务继续。

```bash
reopen claude      # 浏览 Claude Code 会话，按 Enter 恢复
...
如果 PATH 里找不到对应 Agent 的二进制，`reopen` 会打印提示并退出，而不是直接报错。
...
- [**agent-session-cleaner**](https://github.com/haowang02/agent-session-cleaner)——作者 [haowang02](https://github.com/haowang02)。`reopen` 是这个项目的 fork，会话浏览、搜索、清理、归档等能力全部来自原作者。在此致谢。
